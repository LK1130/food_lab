@extends('COMMON.layout.layout_customer_2')
...
<div class="body">
    ...
    <div class="bodyEditProfile d-flex flex-row justify-content-center">
        ...
        <div class="d-flex  me-3 ms-3 mt-0 mb-1 infos">
            ...
            <div class="InputParent">
                <input type="text" name="address" id="address" class="InputChild" value="{{ $user->township_name }}/{{ $user->state_name }}/ ({{ $user->address3 }})" readonly>
            </div>
        </div>
        <div class="d-flex  me-3 ms-3 mt-0 mb-1 infos">
            <i class="fas fa-phone-alt fs-3 me-4 mt-2 text-light" ></i>
            <div class="InputParent">
                <input type="number" name="phonenumber" id="phonenumber" class="InputChild" value="{{ $user->phone }}">
            </div>
        </div>
        <div class="d-flex  me-3 ms-3 mt-0 mb-1 infos">
        ...
        <div class="d-flex  me-3 ms-3 mt-0 mb-1 infos">
            ...
            <div class="InputParent">
                ...
                <input type="text" name="Likes" id="Likes" class="InputChild" data-role="tagsinput" value="{{ $user->favourite_food }}">
            </div>
        </div>
        <div class="d-flex  me-3 ms-3 mt-0 mb-1 infos">
            <i class="fas fa-dizzy fs-3 me-4 mt-2 text-light" ></i>
            <div class="InputParent">
                <input type="text" name="Allergic" id="Allergic" class="InputChild" value="{{ $user->allergic }}">
            </div>
        </div>
        <div class="d-flex  me-3 ms-3 mt-0 mb-1 infos">
            ...
            <div class="InputParent">
                <select name="Taste" id="Taste"  class="InputChild">
                    ...
                    <option value="{{ $taste->id }}" class="text-dark" selected>{{ $taste->taste }}</option>
                    @else
                    <option value="{{ $taste->id }}" class="text-dark" >{{ $taste->taste }}</option>
                    @endif
                    @empty
                    <option  >{{__('message.NoDoctorHasLeft')}} .</option>
                    @endforelse
                </select>
                
            </div>
        </div>
        <button class="btn updateButton">Update Profile</button>
    </div>
</form>
</div>
